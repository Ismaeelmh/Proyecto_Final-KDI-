    ...
    try:
        if window:
            response = await window.fetch(
                "https://saguacate.duckdns.org/guardar_puntuacion",
                {
                    "method": "POST",
                    ...
                    "body": json.dumps({
                        "usuario_id": 1,
                        "juego": "serpiente",
                        "modo": "individual" if modo == 1 else "multijugador",
                        "puntuacion": puntos
                    })
                }
            )
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if bt_salir.collidepoint(mouse_pos):
                        if window:
                           window.guardarYSalir(puntuacion, modo)
                        perdido = False
                        return False
